...
(function (Popcorn) {
  ...
  Popcorn.plugin( "flickr" , function( options ) {
    var containerDiv,
        ...
        _height = options.height || "450px",
        _width = options.width || "640px",
    ...
    var getFlickrData = function() {
      ...
      Popcorn.xhr.getJSONP( _uri, function( data ) {
        ...
        options._slideshow = document.createElement( "div" );
        options._slideshow.classList.add("slideshow");
        window.X = options._slideshow;
        Popcorn.forEach( data.items, function ( item, i ) {
          if ( i < _count ) {
            _link = document.createElement( "a" );
            _link.setAttribute( "href", item.link );
            _link.setAttribute( "target", "_blank" );
            _link.classList.add("transition");
            _link.classList.add("fade");
            ...
            options._slideshow.appendChild( _link );
        ...
        containerDiv.appendChild( options._slideshow );
      });
    };
    ...
    function turnOn( index ) {
      var image = options._slideshow.children[index],
          nextImage = options._slideshow.children[index + 1]
      for(var i=0;i<options._slideshow.children.length;i++) {
        options._slideshow.children[i].classList.remove( "on" );
      }
      image && image.classList.add( "on" );
    }

    //Store the cues so we can destroy them.
    options._cues = [];
    var animationTime = ( options.end - options.start ) / (options.numberofimages);
    for( var count=0; count<options.numberofimages; count++ ) {
      (function( count, _popcorn ) {
        _popcorn.cue( options.start + (count * animationTime), function(){
          turnOn(count);
          console.log( count );
        });
        options._cues[count] = _popcorn.getLastTrackEventId();
      }( count, _popcorn));
    }
    ...
    return {
      ...
      _teardown: function( options ) {
        ...
        //_popcorn.off( "timeupdate", slideShow );
      }
    };
  },
  {
